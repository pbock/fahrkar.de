...
function parseSegments(segmentsFragment) {
	const segments = [ parseSegment(segmentsFragment.slice(15)) ];
	return { data: segments };
}
function parseSegment(buffer) {
	const certificate = buffer.slice(0, 11).toString();
	const validFrom = parseDate(buffer.slice(22, 30).toString());
	const validTo = parseDate(buffer.slice(30, 38).toString(), '2400');

	// The segment ID seems to have been 8 previously and extended to 10 later –
	// in any case, it runs to the first occurrence of "S", so we'll check where
	// that occurs and assign everyhting up until then to the segment ID
	const startOfSBlock = buffer.indexOf(0x53);
	const segmentId = buffer.slice(38, startOfSBlock).toString();
	...
	while (cursor < buffer.length) {
		...
		switch (type) {
			...
			case '031':
				...
				break;
			case '032':
				segment.validTo = parseDate(body, '2400');
				break;
			case '035':
				segment.fromId = body;
				break;
